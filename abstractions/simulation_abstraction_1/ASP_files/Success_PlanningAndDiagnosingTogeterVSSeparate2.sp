#const n = 33. % maximum number of steps.
#const max_len = 32. % maximum activity_length of an activity.
#const max_name = 3.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sorts
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
causes(#physical_exogenous_action,#fluent,#boolean).
...
causes(exo_move(O,L),on_shelf(O,S),false).
...
ab_dc(C) :+ #book(C), -holds(loc(C,kitchen),0), finding_defaults(I).
...
ab_do(O) :-  #object(O), holds(loc(O,R),0), in_room(S,R), not ab_do2(O), finding_defaults(I).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% HISTORY GOES HERE
defined_by_default(on_shelf(book_2,bookshelf_2)).
defined_by_default(on_shelf(book_1,bookshelf_2)).
defined_by_default(loc(cup_1,kitchen)).
defined_by_default(loc(cup_2,kitchen)).
defined_by_default(on_shelf(cup_2,cupboard)).
defined_by_default(on_shelf(cup_1,cupboard)).
obs(loc(rob,lift),true,0).
obs(loc(book_1,lift),false,0).
obs(loc(book_2,lift),false,0).
obs(in_hand(rob,cup_2),false,0).
obs(in_hand(rob,book_2),false,0).
obs(in_hand(rob,book_1),false,0).
obs(in_hand(rob,cup_1),false,0).
obs(loc(cup_1,lift),false,0).
obs(loc(cup_2,lift),false,0).
hpd(select(my_goal), true,0).
obs(my_goal,false,1).
activity_goal(1,my_goal).
activity_component(1,1,move(rob,corridor_2)).
activity_component(1,3,pickup(rob,book_2)).
activity_component(1,10,put_on_shelf(rob,book_2,bookshelf_L)).
activity_component(1,6,move(rob,lift)).
activity_component(1,8,move(rob,corridor_1)).
activity_component(1,2,move(rob,office_2)).
activity_component(1,7,request_move(rob,lift,corridor_1)).
activity_component(1,9,move(rob,library)).
activity_component(1,4,move(rob,corridor_2)).
activity_component(1,5,call_lift(rob,lift,corridor_2)).
activity_length(1,10).
attempt(start(1),1).
attempt(move(rob,corridor_2),2).
obs(in_hand(rob,book_2),false,3).
obs(in_hand(rob,book_1),false,3).
obs(loc(rob,lift),true,3).
obs(loc(book_2,lift),false,3).
obs(loc(book_1,lift),false,3).
attempt(stop(1),3).
