...
''' This is a simulation of the real world. The state of the world is changed and directly_observed by the executer.
...
class World(object):
	...
	def __update_state(self,answer_split):
		...
		for split_fluent in my_fluents_splitted:
			## if my fluent had a comma in it, for example 'loc(rob,kitchen)', then the splitted version is a list with two elements,
			## and  the dictionary will have a key that corresponds to the first element of the list 'loc(rob', with value
	...
	def __getexecutionTimeUnits(self,action):
		if('move' in action):
			return 5
		if('put_on_shelf' in action or 'pickup' in action):
			return 2
		if('request_move' in action or 'call_lift' in action):
			return 1
	...
	def robotObserves(self,fluents_to_observe):
		...
		directly_observed_list = [v for v in answer_split if 'directly_observed' in v]
		return Set(directly_observed_list)
