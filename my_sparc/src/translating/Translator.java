...
public class Translator {
	...
	private void translateRule(ASTprogramRule rule,boolean writeWarningsToSTDERR) throws ParseException {
		...
		if (ra.isCrRule()) {
			...
			appendStringToTranslation(".");
			if(Settings.getSolver() == ASPSolver.Clingo) {
					String n = String.valueOf(Integer.parseInt(ruleName.substring(ruleName.indexOf('_')+1,ruleName.indexOf('(')))+1);
					appendStringToTranslation(" ["+n+"," + ruleName + "]");  	
			}	
			appendNewLineToTranslation();
			ArrayList<ASTatom> newAtoms = new ArrayList<ASTatom>();
			...
			addAtomsToRulesBody(rule, newAtoms);
			try{	
				PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter("/home/rocio/work/crRules.txt", true)));
				//pw.println(" [1," + ruleName + "]");
				pw.println(ruleName);
				String n = String.valueOf(Integer.parseInt(ruleName.substring(ruleName.indexOf('_')+1,ruleName.indexOf('(')))+1);
				pw.println(n);
				pw.println(" ["+n+"," + ruleName + "]");
				pw.println(" [1," + ruleName + "]");

				pw.println(rule.toString(sortRenaming)+"\n");
				pw.close();
				FileWriter clingow = new FileWriter("/home/rocio/work/outputForClingo.txt", true);
				clingow.write(this.translatedOutput.toString());
				clingow.close();
			} 
			catch (Exception ex){
