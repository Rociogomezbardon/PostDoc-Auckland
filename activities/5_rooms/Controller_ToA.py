...
class Controller_ToA(object):
	...
	def getActivity(self,initial_conditions,goal):
		...
		if(answer == "{}\n"): 
			...
			return [time, time_first, time_second, new_activity,activity_name,"Goal Holds",plan_length,len(self.activities_list)]
		elif(answer == "\n"):
			...
			recorded_activity, plan_length = self.__updateActivitiesList(plan)
			self.__updateActivitiesFile()
			if(recorded_activity): activity_name = int(((plan[23:]).split(')'))[0])
			else: activity_name = 0
			new_activity = True
	...
	def __updateActivitiesList(self,answer):
		...
		new_plan = []
		activity_length_entry = answer_split[-1]
		activity_length = int(activity_length_entry[activity_length_entry.rfind(',')+1:-1])
		#Activity length conditions!!!!
		if(activity_length not in [10,11,12]): return False, activity_length
