...
def run_and_record(this_conditions):
	global run
	run += 1
	print "Run: " + str(run)
	initial_conditions_index = run
	initial_conditions_string = parseInitialConditions(this_conditions)
	...
	textfile.write('\n--------------------------------------------------------------\n\n')
	writer.writerow([run, time_traditional, time_toa, time_toa_first, time_toa_second, length_traditional, activity_length, new_activity_toa, activity_name_toa, activities_list_length, initial_conditions_index, goal, goal_holds])
	csvfile.flush()
	...
	for locked_or_not in boolean:
		for robot_location in locations:
			for book1_location in locations:
					...
					run_and_record(initial_conditions)
