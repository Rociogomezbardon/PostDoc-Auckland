...
def createRandomInitialConditions():
	initial_conditions = []
	initial_conditions_index = random.randrange(1,193,1)
	current_index = 0
...
def createConstrainedInitialConditions():
	initial_conditions = []
	locked_or_not = random.choice(boolean)
	robot_location = random.choice(['kitchen', 'library'])
	book1_location = 'office2'
	book2_location = 'office2'
	in_handBook1 = 'false'
	in_handBook2 = 'false'
	initial_conditions = [locked_or_not, robot_location , book1_location , book2_location, in_handBook1, in_handBook2]
	return 0,initial_conditions
...
if __name__ == "__main__":
	sparcPath = "/home/mgom004/work/solverfiles/sparc.jar"
	#sparcPath = "/Users/rociogomez/Documents/Work/Robotics/Software/sparc/sparc.jar"
	...
	goal = 'tidy_all(library)'
	for run in xrange(250):
		initial_conditions = createConstrainedInitialConditions()
		initial_conditions_index = initial_conditions[0]
		initial_conditions_string = parseInitialConditions(initial_conditions[1])

		time_toa, time_toa_first, time_toa_second, new_activity_toa, activity_name_toa, activity_toa, activities_list_length  = controller_toa.getActivity(initial_conditions_string,goal)
		time_traditional, plan_traditional = controller_traditional.getPlan(initial_conditions_string,goal)		

		length_toa = len(activity_toa)
		...
		writer.writerow([run+1, time_traditional, time_toa, time_toa_first, time_toa_second, length_traditional, length_toa, new_activity_toa, activity_name_toa, activities_list_length, initial_conditions_index, goal, goal_holds])
		csvfile.flush()
