...
class World(object):
	...
	def __get_exo_action(self,action):
		exo_action = ''
		my_exo_action = random.choice(list(self.domain_info_formatting.abstract_signature_dic['#exo_action']))
		exo_action_parameters = self.domain_info_formatting.abstract_signature_dic[my_exo_action]
	...
	def __runASPDomain(self,input):
		...
		f1.close()
		print (asp_file)
		answer = subprocess.check_output('java -jar '+ self.domain_info_formatting.sparc_file + ' ' +asp_file+' -A',shell=True)
		if answer == '' or answer == '\n':
	...
	def executeAction(self,action):
		...
		if('occurs('+ exo_action +',0)' in answer_split):
			...
			#TODO: comment out next line - this needs to be commetned out so that our simulation only has one exo action.
