...
ab_d1(B) :+ #book(B), -holds(loc(B,office1),0), diagnosing(I), current_step(I).
...
ab_dL(B) :+ #book(B), -holds(loc(B,library),0), diagnosing(I), current_step(I).
...
-holds(loc(B,office1),0) :- ab_d1(B), not diagnosing(I).
-holds(loc(B,library),0) :- ab_dL(B), not diagnosing(I).
holds(F,0) :- defined_by_default(F), not diagnosing(I).
...
%%(27)
number_explained(N,I) :- current_step(I),
    diagnosing(I),
