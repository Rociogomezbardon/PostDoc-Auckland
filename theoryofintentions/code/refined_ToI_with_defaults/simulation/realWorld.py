...
class World(object):
	def __init__(self,dic_initial_state, new_domain_info,sparc_file):
	...
	def __getRandomExoAction(self,action):
		exo_action = ''
		if self.exo_action_happened or random.random()<0.5: return ''
		object_choice = random.choice(list(self.domain_info.refined_signature_dic['#coarse_object']))
		if ('coarse_in_hand(rob1', object_choice) in self.dic_RefinedState.items(): return ''
		if self.dic_RefinedState['coarse_loc(' + object_choice] == self.dic_RefinedState['coarse_loc(rob1'] : return ''
		location_choice = random.choice(list(self.domain_info.refined_signature_dic['#place']))
	...
	def executeAction(self,action):
		...
		if exo_action != '' : input.append(exo_action)
		...
		if(answer_list and 'occurs('+ action +',0)' in answer_list):
			...
			if exo_action != '' :
				...
				print ('\nEXO ACTION HAPPENNED!!: ' + 	exo_action)
	...
	def __runASPDomain(self,input):
		...
		answer = subprocess.check_output('java -jar '+ self.sparc_file + ' ' +asp_Refined_World_file+' -A',shell=True)
